...
const Lt = (s, t, e, n) => {
  const r = H.ProjectivePoint.fromPrivateKey(I(s.s)), i = n.multiply(F(s.e)), a = t.multiply(F(s.s)), o = e.multiply(F(s.e)), c = r.subtract(i), u = a.subtract(o), l = Ft([c, u, n, e]);
  return Ht(l, s.e);
}, Wt = (s, t, e, n) => {
...
function yt(s, t, e, n) {
  ...
  vt(e, "asc").forEach((u) => {
    const l = i.filter((p) => p === u).length, f = Math.max(n - l, 0);
    for (let p = 0; p < f && !(r.reduce((h, d) => h + d, 0) + u > t); ++p)
      r.push(u);
  });
  const o = t - r.reduce((u, l) => u + l, 0);
  return o && A(o, e).forEach((l) => {
    r.push(l);
  }), r.sort((u, l) => u - l);
}
...
class _e {
   ...
   */
  log(t, e, n) {
    this.logger ? this.logger(t, e, n) : console.log(`[${t}] ${e}`, n);
  }
  /**
...
class K {
  ...
  static createSingleDeterministicData(t, e, n, r) {
    const i = xe(e, r, n), a = U(i), o = new TextEncoder().encode(a), c = ue(Be(e, r, n)), { r: u, B_: l } = J(o, c);
    return new K(
      new V(t, l, r).getSerializedBlindedMessage(),
      u,
      o
    );
  }
...
class Ze {
   ...
   */
  async receive(t, e) {
    const { requireDleq: n, keysetId: r, outputAmounts: i, counter: a, pubkey: o, privkey: c, outputData: u, p2pk: l } = e || {};
    ...
    const d = this.createSwapPayload(
      p,
      t.proofs,
      f,
      i,
      ...
      h,
      l
    ), { signatures: w } = await this.mint.swap(d.payload), y = d.outputData.map((k, m) => k.toProof(w[m], f)), g = [];
    return d.sortedIndices.forEach((k, m) => {
      g[k] = y[m];
    }), g;
  }
  /**
  ...
  async send(t, e, n) {
    ...
    const { keep: h, send: d } = this.selectProofsToSend(
      e,
      t,
      n?.includeFees
    ), w = a ? this.getFeesForProofs(d) : 0;
    if (!i && (x(d) != t + w || // if the exact amount cannot be selected
    u || l || f || c || p)) {
    ...
    if (x(d) < t + w)
      throw new Error("Not enough funds available to send");
    return o ? { keep: h, send: d } : { keep: h, send: $(d) };
  }
  selectProofsToSend(t, e, n) {
    const r = t.sort((h, d) => h.amount - d.amount), i = r.filter((h) => h.amount <= e).sort((h, d) => d.amount - h.amount), o = r.filter((h) => h.amount > e).sort((h, d) => h.amount - d.amount)[0];
    if (!i.length && o)
      return {
    ...
    if (!i.length && !o)
      return { keep: t, send: [] };
    let c = e, u = [i[0]];
    const l = [], f = n ? this.getProofFeePPK(u[0]) : 0;
  ...
  async swap(t, e, n) {
    ...
    const { includeFees: i, keysetId: a, counter: o, pubkey: c, privkey: u, proofsWeHave: l, outputData: f, p2pk: p } = n || {}, h = await this.getKeys(a), d = e;
    let w = t;
    const y = x(e);
    let g = y - w - this.getFeesForProofs(d), k = r?.sendAmounts || A(w, h.keys);
    if (i) {
    ...
    if (w + this.getFeesForProofs(d) > y)
      throw console.error(
    ...
    if (w + this.getFeesForProofs(d) + g != y)
    ...
    const P = f?.keep || this._keepFactory, q = f?.send, O = this.createSwapPayload(
      ...
      { keep: P, send: q },
      p
    ), { signatures: Ct } = await this.mint.swap(O.payload), ht = O.outputData.map((_, v) => _.toProof(Ct[v], h)), lt = [], dt = [], ft = Array(O.keepVector.length), mt = Array(ht.length);
    return O.sortedIndices.forEach((_, v) => {
      ft[_] = O.keepVector[v], mt[_] = ht[v];
    }), mt.forEach((_, v) => {
      ft[v] ? lt.push(_) : dt.push(_);
    }), {
      keep: lt,
      send: dt
    };
  ...
  async restore(t, e, n) {
    ...
    c.forEach((h, d) => l[h.B_] = u[d]);
    const f = [];
    let p;
    for (let h = 0; h < o.length; h++) {
      const d = l[o[h].blindedMessage.B_];
      d && (p = t + h, o[h].blindedMessage.amount = d.amount, f.push(o[h].toProof(d, i)));
    }
    return {
      proofs: f,
  ...
  async mintProofs(t, e, n) {
    let { outputAmounts: r } = n || {};
    const { counter: i, pubkey: a, p2pk: o, keysetId: c, proofsWeHave: u, outputData: l, privateKey: f } = n || {}, p = await this.getKeys(c);
    !r && u && (r = {
    ...
    const { signatures: w } = await this.mint.mint(d);
  ...
  async meltProofs(t, e, n) {
    ...
    }, l = await this.mint.melt(u);
    return {
      quote: { ...l, unit: t.unit, request: t.request },
      change: l.change?.map((f, p) => c[p].toProof(f, o)) ?? []
    };
  }
  /**
  ...
  createSwapPayload(t, e, n, r, i, a, o, c, u) {
    const l = e.reduce((m, P) => m + P.amount, 0);
    r && r.sendAmounts && !r.keepAmounts && (r.keepAmounts = A(
      l - t - this.getFeesForProofs(e),
      n.keys
    ));
    const f = l - t - this.getFeesForProofs(e);
    let p = [], h = [];
    if (c?.keep)
      if (Y(c.keep)) {
        const m = c.keep;
    ...
    const d = [...p, ...h], w = d.map((m, P) => P).sort(
      (m, P) => d[m].blindedMessage.amount - d[P].blindedMessage.amount
    ), y = [
      ...Array(p.length).fill(!0),
      ...Array(h.length).fill(!1)
    ], g = w.map((m) => d[m]), k = w.map((m) => y[m]);
    return {
      payload: {
        inputs: e,
  ...
  async checkProofsStates(t) {
    ...
    for (let a = 0; a < n.length; a += r) {
      ...
      for (let l = 0; l < o.length; l++) {
        const f = u[o[l]];
        if (!f)
          throw new Error("Could not find state for proof with Y: " + o[l]);
        i.push(f);
      }
    }
    return i;
  ...
  createOutputData(t, e, n, r, i, a, o) {
    ...
    } else a ? c = K.createP2PKData(a, t, e, i) : o ? c = A(t, e.keys).map((l) => o(l, e)) : c = K.createRandomData(t, e, i);
    return c;
  }
  /**
...
class Re {
  ...
  async mintProofs(t, e, n) {
    ...
    }, { signatures: o } = await this.mint.mint(a, e), c = i.map((u, l) => u.toProof(o[l], r));
    if (c.some((u) => !xt(u, r)))
