...
const Ft = K("536563703235366b315f48617368546f43757276655f43617368755f");
function H(s) {
  const t = O(D.concat([Ft, s])), e = new Uint32Array(1), n = 2 ** 16;
  for (let r = 0; r < n; r++) {
    const o = new Uint8Array(e.buffer), c = O(D.concat([t, o]));
    try {
...
const Ct = (s) => {
  let t;
  return /^[a-fA-F0-9]+$/.test(s) ? t = _t(s) % BigInt(2 ** 31 - 1) : t = Q(Rt(s)) % BigInt(2 ** 31 - 1), t;
};
function Lt(s, t) {
...
function Zt(s, t, e) {
  const { value: n, offset: r } = R(s, t, e);
...
function se(s, t, e) {
  const { value: n, offset: r } = R(s, t, e), o = [];
  let c = r;
  for (let i = 0; i < n; i++) {
    const a = z(s, c);
...
function le(s) {
  if (s.proofs.forEach((i) => {
    ...
    throw new Error("can not encode to v4 token if proofs contain non-hex keyset id");
  const e = Mt(s), n = st(e), r = "cashu", o = "B", c = jt(n);
  return r + o + c;
}
function Mt(s) {
  const t = {}, e = s.mint;
...
function je(s) {
  const t = Object.entries(s).sort((r, o) => +r[0] - +o[0]).map(([, r]) => K(r)).reduce((r, o) => me(r, o), new Uint8Array()), e = O(t);
...
function ze(s) {
  const t = new TextEncoder(), e = Mt(s), n = st(e), r = t.encode("craw"), o = t.encode("B");
  return we(r, o, n);
}
function Ge(s) {
    ...
    for (const [t, e] of this.storedSubscriptions)
      try {
        const n = this.createSubscription(
          { kind: e.params.kind, filters: e.params.filters },
          e.callback,
          e.errorCallback
        );
      } catch (n) {
        console.error("WSConnection: Failed to resubscribe:", n), e.errorCallback(new Error("Failed to resubscribe after reconnection"));
      }
  }
  sendRequest(t, e) {
...
const Ae = (s) => {
  ...
  const e = O(new TextDecoder().decode(s));
  return nt.sign(e, t);
}, bt = (s, t) => {
  let e = [], n = "";
...
function Me(s, t, e) {
  const n = Ke(t, e), r = X(s), o = nt.sign(n, r);
  return x(o);
}
class J {
  constructor(t, e, n) {
...
const De = "m/129372'/0'", qe = (s, t, e) => xt(
  ...
  const r = Ot.fromMasterSeed(s), o = Ct(t), c = `${De}/${o}'/${e}'/${n}`, i = r.derive(c);
...
class Xe {
  ...
  createOutputData(t, e, n, r, o, c, i) {
    ...
    } else c ? a = M.createP2PKData(c, t, e, o) : i ? a = P(t, e.keys).map((d) => i(d, e)) : a = M.createRandomData(t, e, o);
    return a;
  }
  /**
...
class Ne {
   ...
   */
  async mintProofs(t, e, n) {
    const r = await this.getKeys(n?.keysetId), o = M.createRandomData(t, r), c = {
...
async function Ze(s, t, e) {
  const n = new C(t);
  return (await new Ne(n).mintProofs(s, e)).map((c) => Oe(c));
}
export {
  C as CashuAuthMint,
